...
function OrdersPage() {
  ...
  const load = async () => {
    setBusy(true);
    const data = await adminGet<{ orders: Order[] }>("/api/admin/orders").catch(() => ({ orders: [] }));
  ...
  const updateOrder = async (id: string, patch: Record<string, string>) => {
    setUpdating(id);
    await adminPatch(`/api/admin/orders?id=${id}`, patch).catch(() => { toast.error("Update failed"); });
    setUpdating(null);
    ...
    <div className="space-y-4">
      <div className="bg-card border border-border rounded-xl p-4 flex flex-col md:flex-row gap-3 md:items-center">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by phone, order ID, name, email…" className="w-full pl-9 pr-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary/30" />
        </div>
      ...
      {busy ? (
        ...
        <div className="bg-card border border-border rounded-xl py-16 text-center text-muted-foreground text-sm">No orders match your filters.</div>
      ) : (
...
function OrderRow({ order, updating, onUpdate }: { order: Order; updating: boolean; onUpdate: (p: Record<string, string>) => void }) {
    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden">
      <button onClick={() => setOpen((v) => !v)} className="w-full flex flex-wrap items-center gap-3 p-4 text-left hover:bg-muted/30 transition">
        <div className="flex-1 min-w-[180px]">
          ...
          <div className="text-xs text-muted-foreground">{new Date(order.createdAt).toLocaleString("en-IN")} • #{order.id.slice(0, 8)} • {order.shippingPhone || "—"}</div>
        </div>
