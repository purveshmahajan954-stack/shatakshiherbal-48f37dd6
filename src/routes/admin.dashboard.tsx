...
function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [ordersRes, profilesRes] = await Promise.all([
        supabase
          .from("orders")
          .select("id,subtotal,gst,total,payment_status,status,created_at,shipping_name,email")
  ...
  const stats = useMemo(() => {
    ...
    const sum = (arr: Order[], k: "subtotal" | "gst" | "total") =>
      arr.reduce((s, o) => s + Number(o[k] || 0), 0);
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData}>
              ...
              <Tooltip
                ...
                formatter={(v: number, n: string) => [`₹${v.toLocaleString("en-IN")}`, n === "revenue" ? "Revenue" : "GST"]}
              />
    ...
    <div className="bg-card border border-border rounded-xl p-5">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div>
          ...
          <div className="font-medium">₹{data.base.toLocaleString("en-IN")}</div>
        </div>
        <div>
          <div className="text-xs text-muted-foreground">GST (5%)</div>
          <div className="font-medium text-primary">₹{data.gst.toLocaleString("en-IN")}</div>
        </div>
      </div>
    </div>
  );
...
// Receipt icon referenced for tree-shake safety
void Receipt;
