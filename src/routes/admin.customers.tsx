...
function CustomersPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [profilesRes, ordersRes] = await Promise.all([
        supabase.from("profiles").select("id,full_name,email,created_at").order("created_at", { ascending: false }).limit(2000),
      ...
      const list: CustomerSummary[] = profiles.map((p) => {
        ...
        return {
          profile: p,
          orders: userOrders,
          totalSpent: paid.reduce((s, o) => s + Number(o.total || 0), 0),
          paidOrders: paid.length,
          lastOrder: last,
          phone: last?.shipping_phone || null,
          address: last?.shipping_address || null,
        };
      });
  ...
  if (busy) {
    return <div className="py-16 text-center"><Loader2 className="w-6 h-6 animate-spin text-primary mx-auto" /></div>;
  }
          ...
          {filtered.map((c) => {
            ...
            return (
              <div key={c.profile.id} className="bg-card border border-border rounded-xl">
                <button
                  onClick={() => setOpenId(isOpen ? null : c.profile.id)}
                  className="w-full text-left p-4 flex flex-wrap items-center gap-4 hover:bg-muted/30 transition"
                >
                  <div className="w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center font-semibold uppercase">
                    {(c.profile.full_name || c.profile.email || "?").slice(0, 1)}
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <div className="font-semibold">{c.orders.length}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xs text-muted-foreground">Spent</div>
                    <div className="font-semibold">₹{c.totalSpent.toLocaleString("en-IN")}</div>
                  </div>
                </button>
                  ...
                  <div className="border-t border-border p-4 grid md:grid-cols-2 gap-6 bg-muted/20">
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <div className="col-span-2"><div className="text-xs text-muted-foreground">Customer since</div><div className="font-semibold">{new Date(c.profile.created_at).toLocaleDateString("en-IN", { dateStyle: "medium" })}</div></div>
                      </div>
                    </div>
                    <div>
                          ...
                          {c.orders.map((o) => (
                            <li key={o.id} className="p-2.5 flex items-center gap-2">
                              ...
                              <span className="flex-1 truncate">
                                ...
                                <span className="text-muted-foreground ml-2 text-xs">{new Date(o.created_at).toLocaleDateString("en-IN")}</span>
                              </span>
                              <span className="text-xs px-2 py-0.5 rounded-full bg-muted">{o.payment_status}</span>
